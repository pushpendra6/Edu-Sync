    ...
    <>
      ...
      {showPopup && (
        <div className="position-fixed top-0 start-0 w-100 h-100 bg-dark bg-opacity-75 d-flex align-items-center justify-content-center">
          <div className="bg-white p-4 rounded shadow text-center">
            {popupType === "submit" && (
              <>
                ...
                <button className="btn btn-success m-2" onClick={handlePopupConfirm}>Submit</button>
                <button className="btn btn-secondary m-2" onClick={handlePopupCancel}>Continue</button>
              </>
            )}
            {popupType === "end" && (
              <>
                <h4>You still have time left. End quiz?</h4>
                <button className="btn btn-danger m-2" onClick={handlePopupConfirm}>End Test</button>
                <button className="btn btn-secondary m-2" onClick={handlePopupCancel}>Continue</button>
              </>
            )}
            {popupType === "timeout" && (
              <>
                <h4>Time is over! Your quiz has Submitted results will be declared soon.</h4>
                <button className="btn btn-warning mt-3" onClick={handlePopupConfirm}>Go Back</button>
              </>
            )}
          </div>
        </div>
          ...
          { quizData.length > 0 ? (<button className="btn btn-info" onClick={handleSubmit}>
						submit
					</button>) : (<Link className="btn btn-info" to='/student-dashboard/quiz'>
						Back
					</Link>)}
